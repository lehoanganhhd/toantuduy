...
function my_social($attrs, $content=null,$shortcodename="") {
	...
	extract(shortcode_atts(array('position'=>'' ),$attrs));
	...
	if( isset($dttheme_options['general']['show-sociables']) && isset($dttheme_options['social']) ):
		$out = "<ul class='dt-sc-social-icons'>";
			foreach($dttheme_options['social'] as $social):
				...
				if( $position === "header" ):
					$class = ( $class == 'vimeo' ) ? 'vimeo-square' : $class;
					$out .= "<li>";
					$out .= "<a class='dt-sc-tooltip-top {$class}' href='{$link}' title='{$title}' target='_blank'>";
					$out .= "<span class='fa fa-{$class}'></span>";
				else:
					$out .= "<li class='{$class}'>";
					$out .= "<a href='{$link}' target='_blank' title='{$title}'>";
